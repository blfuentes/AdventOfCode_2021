...
let rec performStep(current:int, steps: int, myPolymer: string, trans: string[] list) =
    ...
    | true -> myPolymer
    | false -> 
        //printfn "Running step: %i with string of %i elements" current myPolymer.Length
...
let resultstring = performStep(0, 40, polymer, replaceTransitions)
...
for idx in 1..40 do
     let resultstring = performStep(0, idx, polymer, replaceTransitions)
     let counters = resultstring.ToCharArray() |> Array.countBy(fun c -> c) |> Array.toList
     let result = (snd((counters |> List.sortByDescending(fun c -> snd c) |> List.head)))
                     - (snd ((counters |> List.sortBy(fun c -> snd c) |> List.head)))   
     printfn "Step %i = %i" idx result

let resultstring = performStep(0, 1, polymer, replaceTransitions)
...
let counters = resultstring.ToCharArray() |> Array.countBy(fun c -> c) |> Array.toList
let max = ((snd((counters |> List.sortByDescending(fun c -> snd c) |> List.head))) |> bigint)
let min = ((snd ((counters |> List.sortBy(fun c -> snd c) |> List.head))) |> bigint)
let numberOfElements = (resultstring.Length |> bigint) * (Math.Pow(2, step - 1.0) |> bigint) - (Math.Pow(2, step - 1.0) |> bigint) + 1I
let rMax = max * numberOfElements / (resultstring.Length |> bigint)
let rMin = min * numberOfElements / (resultstring.Length |> bigint)
rMax - rMin
